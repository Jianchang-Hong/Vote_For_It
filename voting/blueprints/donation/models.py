...
def get_all_charities():
    ...
    with Cursor(dictionary=True) as cursor:
        cursor.execute("""
            SELECT c.*, t.theme_name FROM charities c, themes t WHERE c.theme_id = t.theme_id
        """)
...
def get_all_approved_charities():
    ...
    with Cursor() as cursor:
        cursor.execute(
            '''SELECT ch.*, u.username as created_by_username, t.theme_name as theme_name
            FROM charities ch, users u, themes t WHERE ch.create_by = u.user_id 
            AND ch.approved = 1 AND ch.theme_id = t.theme_id order by ch.created_at desc'''
        )
        charities = cursor.fetchall()
    return charities
